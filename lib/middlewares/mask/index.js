  ...
  return {
        ...
        strapi.app.use(async (ctx, next) => {
          ...
          const mask = (payload) => {
            ...
            if (_.isArray(payload)) {
              ...
              // if( payload.private ) {
                // console.log('plugin->', ctx.request.route.plugin, ' -->', payload);
              // }
              return this.mask(
                ctx,
                Object.keys(payload).reduce((acc, current) => {
                  acc[current] = current !== 'private' && _.isObjectLike(payload[current]) ? mask(payload[current]) : payload[current];
    ...
    filteredModels: function (matchs) {
      return matchs.reduce((acc, match, index) => {
        ...
        if (match.model === 'user' || match.intersection >= Object.keys(attributes).length / 2) {
          acc[index] = match;
        }
        // acc[index] = match;
